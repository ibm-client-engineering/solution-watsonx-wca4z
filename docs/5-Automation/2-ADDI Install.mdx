...
Next, update the ```.env``` files by adding the ADDI host's external IP address (or FQDN if possible) as the value to the CCS_IP. Update the CCS_PORT value if the default is already used.
