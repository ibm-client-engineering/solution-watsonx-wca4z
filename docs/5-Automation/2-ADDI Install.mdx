...
First ensure OpenSSL is installed. An install script exists in the tls folder
```powershell
.\dependencies\install_open_ssl.ps1
```

Next, update the `tls/.env` file according to your environment
```bash
keyPass="p@ssw0rd"
...
Then you can run to set up the following certs `server.key`, `server_certificate`, `server_keystore`, `zookeeper`, `root.crt` in `C:\certificates\`
```
