...
Make sure that `C:\certificates` exists as this is where we will store these keys.

In our example environment we do not have DNS configured and our host technically does not have an FQDN, so we will be using the host ip address which is `10.241.64.4`. In an actual client configuration we would recommend a FQDN but for now we are going to use the host ip as the alias. We're also setting our key password to `p@ssw0rd`.

```powershell
keytool -genkeypair -alias "10.241.64.4" -keyalg RSA -keysize 2048 -dname "cn=10.241.64.4" -keypass "p@ssw0rd" -keystore "C:\certificates\server_keystore.p12" -storepass "p@ssw0rd" -storetype PKCS12 -ext BasicConstraints:critical=ca:true -ext san=dns:10.241.64.4
```
...
```powershell
keytool -exportcert -alias "10.241.64.4" -keystore "C:\certificates\server_keystore.p12" -file "C:\certificates\server_certificate.crt" -storepass "p@ssw0rd"
```

Now let's re-import the certificate into the keystore under the alias of `self-signed-root`

```powershell
keytool -keystore "C:\certificates\server_keystore.p12" -import -file "C:\certificates\server_certificate.crt" -alias "self-signed-root" -storepass "p@ssw0rd"
```

If you have the DB2 SSL certificate for the cloud instance of DB2, import it into the keystore as well.

```powershell
keytool -import -trustcacerts -alias "DB2-ssl-cert" -file "C:\certificates\DigiCertGlobalRootCA.crt" -keystore "C:\certificates\server_keystore.p12" -storepass "p@ssw0rd"
```

Delete the `server_certificate.crt` file as this is not needed now and we are apparently going to recreate it.
...
```bash
openssl pkcs12 -in "C:\certificates\server_keystore.p12" -nocerts -nodes -out "C:\certificates\server.key"
```

Generate the `server_certificate.crt` file again, but _different_ in your Powershell window.

```tsx
keytool -list -keystore "C:\certificates\server_keystore.p12" -rfc > "C:\certificates\server_certificate.crt"
```
...
Under `C:\Program Files\IBM Application Discovery and Delivery Intelligence\Authentication Server (DEX)\conf`

Our `issuer` should be either the FQDN or the IP of the ADDI host

We're going update the `dex.yaml` with the following
```yaml
issuer: https://10.241.64.4:7600/dex
storage:
  type: sqlite3
  config:
  file: dex.db
...
At the bottom go to "Drag and Drop files here..." and upload the following files from `C:\certificates`
- `server_keystore.p12`
