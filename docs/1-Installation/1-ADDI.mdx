...
Our alias here is our hostname, which in this case is `wca4z-addi`. We're also setting our key password to `p@ssw0rd`.

```powershell
keytool -genkeypair -alias "wca4z-addi" -keyalg RSA -keysize 2048 -dname "cn=wca4z-addi" -keypass "p@ssw0rd" -keystore "C:\certificates\server_keystore.p12" -storepass "p@ssw0rd" -storetype PKCS12 -ext BasicConstraints:critical=ca:true -ext san=dns:wca4z-addi
```
...
```powershell
keytool -exportcert -alias "wca4z-addi" -keystore "C:\certificates\server_keystore.p12" -file "C:\certificates\server_certificate.crt" -storepass "p@ssw0rd"
```
...
```powershell
keytool -import -trustcacerts -alias "DB2-ssl-cert" -file "C:\certificates\<DB2 SSL certificate file> -keystore server_keystore.p12 -storepass "p@ssw0rd"
```
...
Generate the `server_certificate.crt` file again, but _different_.
...
#### Configuring the DEX server on ADDI host
