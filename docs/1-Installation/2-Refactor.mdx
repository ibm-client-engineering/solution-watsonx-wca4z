...
```powershell
keytool -importcert -alias ad-core-server -keystore "C:\certificates\server_keystore.p12" -storetype PKCS12 -storepass "p@ssw0rd" -file "C:\certificates\root.crt"
```
...
Presumably once we have a DB2 instance on cloud, we will get another cert that will have to be imported and installed as well.
...
1. Open a Command Prompt (on Windows) or a Terminal (on Linux) and go to the folder containing the newly prepared server_keystore.p12.
...
Export the zookeeper.crt from the keystore... (not sure what the deal is with this). I think this is just literally the same server_cert we created before, but we're calling it zookeeper instead.

```powershell

keytool -exportcert -alias "wca4z-addi" -keystore "C:\certificates\server_keystore.p12" -rfc -file "C:\certificates\zookeeper.crt" -storepass "p@ssw0rd" -ext BasicConstraints:critical=ca:true -ext san=dns:wca4z-addi

```

Ok, this is what we're importing to our Refactor linux host. So let's just copy the contents over.

Now let's make some copies on the refactor host
